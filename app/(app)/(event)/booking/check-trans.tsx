...
export default function CheckEventSeatTransScreen() {
    ...
    const trans = useStoreTransactionEventSeat(s => s.trans);
    ...
    const saveQRcode = useCallback(async (url: string) => {
        ...
        try {
            ...
            const qrFile = new File(destination, `event_seat_qr_${Date.now()}_${Math.floor(Math.random() * 10000)}.png`);
    ...
    useEffect(() => {
        if (!trans) {
            ...
            router.replace("/(app)/(event)/booking/seats");
        }
    }, [trans, t]);
