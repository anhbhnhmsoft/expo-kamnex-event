...
export default function CheckDocumentPaymentScreen() {
    useDisableBackGesture();

    const trans = useStoreTransactionDocument(s => s.trans);
    ...
    const saveQRcode = useCallback(async (url: string) => {
        ...
        try {
            ...
            const qrFile = new File(destination, `document_payment_qr_${Date.now()}_${Math.floor(Math.random() * 10000)}.png`);
